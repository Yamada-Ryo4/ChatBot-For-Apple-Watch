...
- **⚡️ 智能上下文管理**
  - **滑动窗口**: 发送请求时自动截取最近 10 条消息作为上下文，既保证对话连贯性，又节省 Token、防止超限。
...
1. 进入 **Settings** -> **数学公式渲染** 区域。
2. **启用 LaTeX 渲染**: 开启后，数学公式 (如 `\frac{a}{b}`, `\sqrt{2}`) 会被转换为可读格式。
3. **高级渲染模式** (可选): 开启后使用更精细的图文混排渲染。
